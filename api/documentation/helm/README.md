...
The Helm chart installs bgs and by default the following dependencies using subcharts:
...
```bash
helm install my-bgs  --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `bgs` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `bgs` |
